import pygame
import random
from wordle.constants import WIDTH, HEIGHT, GREEN, YELLOW, WHITE, BLACK
from wordle.UI import Board, FONT
pygame.init()
...
def word_exists(words_list, guess, entry_field, turn):
    if turn > 0:
        turn -= 1
        guess[turn] = entry_field[turn][0] + entry_field[turn][1] + entry_field[turn][2] + entry_field[turn][3] + entry_field[turn][4]
    ...
    if word_in_dict == True:
            for y in range(0, 5):
                for x in range(0, 6):
                    for z in range(10):
                        row, shift = 0, 20
                        Board.indicator_color(WIN, entry_field, turn, real_word_index, x, y, z, row, shift)
                    for z in range(9):
                        row, shift = 1, 40
                        Board.indicator_color(WIN, entry_field, turn, real_word_index, x, y, z, row, shift)
                    for z in range(7):
...
def letter_check(real_word, entry_field, turn, guess, word_in_dict):
    ...
    if word_in_dict == True:
        for y in range(0, 5):
            for x in range(0, 6):
                guess[x] = entry_field[x][0] + entry_field[x][1] + entry_field[x][2] + entry_field[x][3] + entry_field[x][4]
...
def main():
    words = open("C:/Users/kimba/wordlegame/five_letters.csv")
    words_list = []
    for row in words:
    ...
    while run:
        ...
        game_over = letter_check(real_word, entry_field, turn, guess, word_in_dict)
